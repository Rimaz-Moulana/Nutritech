...
function RulePopup({ rules, onClose }) {
  return (
          ...
          <div className="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
            <div className="sm:flex sm:items-start">
              <div className="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                <h3 className="text-lg font-medium leading-6 text-gray-900">Regulations</h3>
                <div className="mt-2">
                  {rules.map((rule, index) => (
                    <>
                    <p key={index} className="text-sm text-black font-semibold mt-4">Regulation {rule.ruleNumber}</p>
                    <p key={index} className="text-sm text-gray-500">{rule.rule}</p>
                    </>
                  ))}
                </div>
              </div>
