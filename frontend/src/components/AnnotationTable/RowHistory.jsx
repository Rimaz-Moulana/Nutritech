    ...
    function RowHistory({ videoId }) {
      ...
      useEffect(() => {
        const fetchAnnotations = async () => {
          try {
            const response = await axios.get(`http://localhost:3001/annotationhistory/${videoId}`);
      ...
      {annotations.map((annotation, index) => (
        <div key={index} className='flex'>
          ...
          <div className="relative mt-4 min-w-[200px] border-gray-200 border-1 rounded-sm">
            <label id={`rule-${index}`} className="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Rule</label>
            <p >{annotation.rule}</p>
          </div>
          </div>
          ...
          <label id={`details-${index}`} className="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Details</label>
            <p className='block w-96 h-52 p-2 text-gray-900 border border-gray-300 rounded-lg bg-gray-50 sm:text-xs focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500'>{annotation.details}</p>
          </div>
          <div className="relative h-full mb-6 ml-12 border-gray-200 border-1 rounded-sm">
            ...
            <p className='block w-96 h-52 p-2 text-gray-900 border border-gray-300 rounded-lg bg-gray-50 sm:text-xs focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500'>{annotation.recommendation}</p>
          </div>
        </div>
      ))}
    </div>
